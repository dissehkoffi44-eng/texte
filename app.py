...
from pydub import AudioSegment# Force FFMPEG (Windows)
if os.path.exists(r'C:\ffmpeg\bin'):
    os.environ["PATH"] += os.pathsep + r'C:\ffmpeg\bin'# ────────────────────────────────────────────────
# CONFIG & CONSTANTS
# ────────────────────────────────────────────────
st.set_page_config(page_title="RCDJ228 MUSIC SNIPER M5", page_icon="", layout="wide")TELEGRAM_TOKEN = st.secrets.get("TELEGRAM_TOKEN")
CHAT_ID        = st.secrets.get("CHAT_ID")NOTES_LIST = ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B']
...
def butter_lowpass(y, sr, cutoff=160):
    ...
    return lfilter(b, a, y)def apply_precision_filters(y, sr):
    y_harm, _ = librosa.effects.hpss(y, margin=(1.3, 4.8))
    nyq = 0.5 * sr
    b, a = butter(4, [50/nyq, 5200/nyq], btype='band')
    return lfilter(b, a, y_harm)def vote_profiles(chroma, chroma_cens, bass_chroma):
    cv   = (chroma       - chroma.min())       / (chroma.max()       - chroma.min()       + 1e-9)
    cens = (chroma_cens  - chroma_cens.min())  / (chroma_cens.max()  - chroma_cens.min()  + 1e-9)
    bv   = (bass_chroma  - bass_chroma.min())  / (bass_chroma.max()  - bass_chroma.min()  + 1e-9)scores = {f"{n} {m}": 0.0 for n in NOTES_LIST for m in ["major", "minor"]}
...
del y, y_filt
gc.collect()
return resultdef get_chord_test_js(btn_id, key_str):
...
# INTERFACE
# ────────────────────────────────────────────────
st.title(" RCDJ228 MUSIC SNIPER M5 — Précision & Granularité 2026")uploaded_files = st.file_uploader("Déposez vos tracks (mp3, wav, flac, m4a)", 
                                 type=['mp3','wav','flac','m4a'], 
                                 accept_multiple_files=True)with st.sidebar:
    st.image("https://cdn-icons-png.flaticon.com/512/2569/2569107.png", width=100)
    st.header("Sniper M5")
    st.caption("Hybride 2026 • Précision M4 + Timeline fine + boost conf ≥80%")st.subheader("Réglages fins")
...
if uploaded_files:
    ...
    results = st.container()for idx, file in enumerate(uploaded_files):
    percent = (idx + 1) / total
    ...
    if data:
        with results:
            ...
            with col1:
                st.markdown(f"<div class='metric-box'><b>TEMPO</b>
<span style='font-size:2.6em;color:#10b981;'>{data['tempo']}</span>
BPM</div>", unsafe_allow_html=True)
            with col2:
                st.markdown(f"<div class='metric-box'><b>ACCORDAGE</b>
<span style='font-size:2.6em;color:#58a6ff;'>{data['tuning_hz']}</span>
Hz ({data['tuning_cents']:+.1f}¢)</div>", unsafe_allow_html=True)
            with col3:
